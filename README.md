...
- **Usage & costs breakdown window** - A tray-menu item opens per-model and per-project cost/token tables for Today / last 7 days / last 30 days — *where* the usage went, not just how much. Cache tokens are shown separately, dated model variants merge into one row, projects appear by their real folder path, and the whole breakdown exports to **CSV** for a spreadsheet. The window is resizable and maximisable — both tables grow to fill it, so a month of usage doesn't have to be read through a small viewport
