...
- **Usage & costs breakdown window** - A tray-menu item opens per-model and per-project cost/token tables for Today / last 7 days / last 30 days — *where* the usage went, not just how much. Cache tokens are shown separately, dated model variants merge into one row, projects appear by their real folder path, and the whole breakdown exports to **CSV** for a spreadsheet. The window is resizable and maximisable — both tables grow to fill it, so a month of usage doesn't have to be read through a small viewport. Click any column header to sort by it (click again to reverse); each table sorts independently, on the real numbers rather than the abbreviated text, with the Total row pinned to the bottom. Select a row to drill into it: pick a model and the project table becomes "Projects using claude-fable-5" (pick a project and the model table becomes "Models used in …"), so you can see *which* projects a model's spend came from — the drill-down follows the timeframe, sorts like the main tables, and its Total matches the row you selected; "Show all" (or selecting the Total row) returns to the full picture. A **Chart** tab plots cost per day over the same timeframe — steady spend and one expensive afternoon add up to the same total but look nothing alike — with dated and dollar axes, and hatched bars on days whose cost is a floor because an unpriced model contributed. Tables stays the default tab, so the window is as compact as it ever was
